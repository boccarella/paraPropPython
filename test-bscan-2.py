...
for i in range(nDepths):
    for j in range(nRanges):
        rxList.append(rx(range_list[j], depth_list[i]))
nRX = len(rxList)
def print_receiver(rx):
    return str(rx.x) + '\t' + str(rx.z)

fout_receivers = open('bscans/receiverList2.txt', 'w+')
...
bscan_array = create_memmap('bscans/bscan-pure-ice-water-pocket2.npy', dimensions=(nDepths, nDepths, nRanges, nSamples), data_type='complex')
# nDepths-> TX, nDepths, nRanges -> RX, nSamples
np.save('bscans/tspace.npy', tspace)
bscan_parallel = create_memmap('bscans/bscan-pure-ice-water-pocket-parallel.npy',  dimensions=(nDepths, nSamples), data_type='complex')
for i in range(nDepths):
    ...
    for j in range(nDepths):
        for k in range(nRanges):
            rx_jj = rx_pulses[jj]
            bscan_array[i][j][k] = rx_jj.spectrum
            if rx_jj.z == sourceDepth and rx_jj.x == range_list[0]:
                bscan_parallel[i] = rx_jj.spectrum
            jj += 1
